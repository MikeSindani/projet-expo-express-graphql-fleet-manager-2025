...
import { AlertCircle, ArrowLeft, Edit2, FileText, Image as ImageIcon, Printer, Trash2 } from 'lucide-react-native';
...
export default function ReportDetailScreen() {
  ...
  console.log("--- Report Detail Debug ---");
  console.log("Requested ID:", id, "Type:", typeof id);
  console.log("Rapports count:", rapports.length);
  if (!rapport) {
    console.log("Available IDs:", rapports.map(r => r.id).slice(0, 10)); // Show firs 10
  } else {
    console.log("Found rapport:", rapport.id);
  }
  ...
  if (!rapport) {
    return (
      <View className={`flex-1 justify-center items-center ${isDark ? 'bg-gray-950' : 'bg-gray-50'} p-4`}>
        ...
        <Text className={`text-center mb-6 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
          Le rapport demandé n'existe pas ou vous n'avez pas la permission de le voir. (ID: {id})
        </Text>
        <TouchableOpacity
          onPress={() => router.back()}
      ...
      <SafeAreaView className={`flex-1 ${isDark ? 'bg-gray-950' : 'bg-gray-50'}`}>
        <ScrollView className="flex-1 px-4 py-6">
          <View className="flex-row items-center justify-between mb-6">
            ...
            <TouchableOpacity onPress={printReport} className="bg-blue-100 dark:bg-blue-900/40 p-3 rounded-2xl">
              ...
              <View>
                <View className="flex-row items-center justify-between">
                  ...
                  <View className={`px-3 py-1 rounded-full ${
                    rapport.type === 'MAINTENANCE' ? 'bg-green-100 dark:bg-green-900/30' :
                    rapport.type === 'REPARATION' ? 'bg-orange-100 dark:bg-orange-900/30' :
                    'bg-blue-100 dark:bg-blue-900/30'
                  }`}>
                    <Text className={`text-[10px] font-black uppercase tracking-wider ${
                      rapport.type === 'MAINTENANCE' ? 'text-green-600 dark:text-green-400' :
                      rapport.type === 'REPARATION' ? 'text-orange-600 dark:text-orange-400' :
                ...
                <Text className="text-gray-500 dark:text-gray-400 text-sm">{chauffeur?.name || 'Chauffeur inconnu'}</Text>
              </View>
            </View>
            ...
            <View className="space-y-6">
              <View className="flex-row gap-4">
                 <View className="flex-1">
                  <Text className="text-gray-500 dark:text-gray-400 text-xs uppercase font-bold mb-1 ml-1">Véhicule</Text>
                  <View className="bg-gray-50 dark:bg-gray-800/50 p-3 rounded-xl border border-gray-100 dark:border-gray-700">
                    <Text className={`font-semibold ${isDark ? 'text-white' : 'text-gray-900'}`}>
                      {vehicule ? `${vehicule.marque} ${vehicule.modele}` : 'Inconnu'}
                    </Text>
                    <Text className="text-gray-500 dark:text-gray-400 text-xs">{vehicule?.immatriculation || '---'}</Text>
                  </View>
                </View>
                <View className="flex-1">
                  <Text className="text-gray-500 dark:text-gray-400 text-xs uppercase font-bold mb-1 ml-1">Kilométrage</Text>
                  {isEditing ? (
                    <TextInput
                      value={kilometrage}
                      onChangeText={setKilometrage}
                      keyboardType="numeric"
                      className={`p-3 rounded-xl border ${isDark ? 'bg-gray-800 border-gray-700 text-white' : 'bg-gray-50 border-gray-200 text-gray-900'} font-bold`}
                    />
                  ) : (
                    <View className="bg-blue-50 dark:bg-blue-900/20 p-3 rounded-xl border border-blue-100 dark:border-blue-900/40">
              ...
              <View>
                <Text className="text-gray-500 dark:text-gray-400 text-xs uppercase font-bold mb-1 ml-1">Incidents</Text>
                {isEditing ? (
                  <TextInput
                    value={incidents}
                    ...
                    className={`p-4 rounded-xl border ${isDark ? 'bg-gray-800 border-gray-700 text-white' : 'bg-gray-50 border-gray-200 text-gray-900'} min-h-[80px]`}
                  />
                ) : (
                  <View className={`p-4 rounded-xl border ${rapport.incidents ? 'bg-orange-50 border-orange-100 dark:bg-orange-900/20 dark:border-orange-900/40' : 'bg-gray-50 border-gray-100 dark:bg-gray-800/50 dark:border-gray-700'}`}>
                    {rapport.incidents ? (
                      ...
                      <Text className="text-gray-400 dark:text-gray-500 italic">Aucun incident</Text>
                    )}
                  </View>
                )}
                ...
                {!isEditing && (
                  <View className="mt-4">
                    <Text className="text-gray-500 dark:text-gray-400 text-xs uppercase font-bold mb-2 ml-1">Photos de l'incident</Text>
                    {rapport.images && rapport.images.length > 0 ? (
                      <ScrollView horizontal showsHorizontalScrollIndicator={false} className="flex-row">
                        {rapport.images.map((photo: any) => (
                          <View key={photo.id} className="mr-3 w-40 h-40 rounded-xl overflow-hidden bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
                            <Image
                      ...
                      <View className="p-4 bg-gray-50 dark:bg-gray-800/30 rounded-xl items-center border border-dashed border-gray-300 dark:border-gray-700">
                        <ImageIcon size={20} color={isDark ? '#4b5563' : '#9ca3af'} />
                        <Text className="text-gray-400 dark:text-gray-500 text-xs mt-1">Aucune photo de l'incident</Text>
                      </View>
                    )}
                  </View>
                )}
              ...
              <View>
                <Text className="text-gray-500 dark:text-gray-400 text-xs uppercase font-bold mb-1 ml-1">Commentaires</Text>
                {isEditing ? (
                  <TextInput
                    value={commentaires}
                    ...
                    className={`p-4 rounded-xl border ${isDark ? 'bg-gray-800 border-gray-700 text-white' : 'bg-gray-50 border-gray-200 text-gray-900'} min-h-[100px]`}
                  />
                ) : (
                  <View className="bg-gray-50 dark:bg-gray-800/50 p-4 rounded-xl border border-gray-100 dark:border-gray-700">
              ...
              {isEditing ? (
                ...
                <View className="space-y-3">
                  ...
                  >
                    ...
                    <Text className="text-red-700 dark:text-red-400 font-bold ml-2">Supprimer le rapport</Text>
                  </TouchableOpacity>
                </View>
              )}
            </View>
